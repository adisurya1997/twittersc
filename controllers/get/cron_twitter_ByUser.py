...
def handler(request, jsonify):
    ...
    try:
        ...
        keyword = requests.get(
            localUrl +
            "/api/statusCronTests?select=appScraper,status,keyword,limit,createdBy&where[status]=ACTIVE&where[appScraper]=Twitter&where[searchBy]=USER",
            headers=header)
